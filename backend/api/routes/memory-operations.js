...
function isMemoryOperationRoute(pathname, method) {
  return pathname === '/api/operations/memory/run' && method === 'POST';
}
async function handleMemoryOperationRequest(req, res, { operationsCoordinator, getIdentity = getClienteCeroIdentity } = {}) {
  ...
  try {
    ...
    if (!operationsCoordinator || typeof operationsCoordinator.runMemoryReview !== 'function') return sendJson(res, 500, { ok: false, code: 'memory_service_unavailable', message: 'La revisión de memoria no está disponible.' });
    return sendJson(res, 200, await operationsCoordinator.runMemoryReview({ identity }));
  } catch (error) {
    if (error && ['operation_in_progress', 'memory_operation_in_progress'].includes(error.code)) return sendJson(res, 409, { ok: false, code: 'operation_in_progress', message: 'Ya existe una operación en curso.' });
    if (error && error.code === 'memory_review_timeout') return sendJson(res, 504, { ok: false, code: error.code, message: 'La revisión tardó más de lo permitido y se detuvo de forma segura.' });
    ...
    return sendJson(res, 500, { ok: false, code: 'memory_review_failed', message: 'No se pudo completar la revisión de memoria.' });
  }
}

module.exports = { handleMemoryOperationRequest, isMemoryOperationRoute };
