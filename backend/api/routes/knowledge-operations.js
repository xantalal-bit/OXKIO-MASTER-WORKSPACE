...
function isKnowledgeOperationRoute(pathname, method) {
  return pathname === '/api/operations/knowledge/run' && method === 'POST';
}
async function handleKnowledgeOperationRequest(req, res, { operationsCoordinator, getIdentity = getClienteCeroIdentity } = {}) {
  ...
  try {
    ...
    if (!operationsCoordinator || typeof operationsCoordinator.runKnowledgeReview !== 'function') return sendJson(res, 500, { ok: false, code: 'knowledge_service_unavailable', message: 'La revisión de conocimiento no está disponible.' });
    return sendJson(res, 200, await operationsCoordinator.runKnowledgeReview({ identity }));
  } catch (error) {
    if (error && ['operation_in_progress', 'knowledge_operation_in_progress'].includes(error.code)) return sendJson(res, 409, { ok: false, code: 'operation_in_progress', message: 'Ya existe una operación en curso.' });
    if (error && error.code === 'knowledge_review_timeout') return sendJson(res, 504, { ok: false, code: error.code, message: 'La revisión tardó más de lo permitido y se detuvo de forma segura.' });
    ...
    return sendJson(res, 500, { ok: false, code: 'knowledge_review_failed', message: 'No se pudo completar la revisión de conocimiento.' });
  }
}
module.exports = { handleKnowledgeOperationRequest, isKnowledgeOperationRoute };
