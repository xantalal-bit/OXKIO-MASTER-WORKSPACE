...
- Objetivo inmediato: 5C.7B.3D sigue abierta como contenedor con 3D.1 y 3D.2
  ...
  **B4.B.2 CERRADA (20/08/2026) — PASS REAL**: `USAGE ON SCHEMA oxkio`
  ...
  demostrada en B4.D.2 (solo `approve()`). B4.F CERRADA (06/09/2026 — PASS REAL DE SANEAMIENTO DE PUENTES TEMPORALES); B5 CERRADA (06/09/2026 — PASS); B5.1 CERRADA OFFLINE — composición segura Approval PostgreSQL preparada y auditada; B5.2 CERRADA OFFLINE — wiring productivo Approval preparado y auditado, pero no activado; B5.3 CERRADA OFFLINE — config/startup productivo fail-closed demostrado sin red; B6 no abierta (**SUPERSEDED 07/09/2026 — B6 / 5C.7B.3F cerrada PASS REAL; ver documento canónico: XANTALAL/00_GOVERNANCE/5C.7B-ARQUITECTURA-EJECUTABLE-RUNTIME.md, sección «Cierre canónico B6 / 5C.7B.3F — 07/09/2026»**); Mission Queue no tocada (ver governance doc,
...
- 5C.7B.3D: abierta como contenedor; 3D.1, 3D.2, 3D.3 y **3D.4 cerradas**; la puerta
  ...
  **B4.B CERRADA** (19/08/2026, puente de privilegios, PASS REAL); **B4.B.2
  ...
  B4.D.2); B4.F CERRADA (06/09/2026 — PASS REAL: retirados exclusivamente los dos puentes temporales de B4.B, con verify final); B5 CERRADA (06/09/2026 — PASS); B5.1 CERRADA OFFLINE — factory/composición segura PostgreSQL validada sin conexión real; B5.2 CERRADA OFFLINE — wiring productivo preparado con JSON como default y PostgreSQL solo por selección explícita, sin conexión real; B5.3 CERRADA OFFLINE — secreto PG-APR condicional y startup PostgreSQL sintético/fail-closed demostrados sin red; B6 no abierta (**SUPERSEDED 07/09/2026 — B6 / 5C.7B.3F cerrada PASS REAL; ver documento canónico: XANTALAL/00_GOVERNANCE/5C.7B-ARQUITECTURA-EJECUTABLE-RUNTIME.md, sección «Cierre canónico B6 / 5C.7B.3F — 07/09/2026»**).
...
- Estado: 5C.7B.1 CERRADA Y PUBLICADA; 5C.7B.2 CERRADA Y PUBLICADA,
  PERSISTENCIA PRINCIPAL RATIFICADA. B6 / 5C.7B.3F CERRADA — PASS REAL (07/09/2026). **4F = CERRADA / PASS**; **5C.7B.4 = CERRADA / PASS REAL / PUBLICADA** (POST-CLOSE, 08/09/2026). Rollback Cloud Run definido y **NO ejecutado**; las limitaciones técnicas pendientes siguen vigentes. **5C.7B.5 = CERRADA / PASS** (08/09/2026 — 5C.7B.5A CERRADA/PASS READ-ONLY, 5C.7B.5B CERRADA/PASS READ-ONLY, 5C.7B.5C CERRADA/PASS DOCUMENTAL; sin creación real de alertas, uptime checks, dashboards, métricas custom, sinks, budgets ni APIs adicionales; ver documento canónico, sección «Cierre canónico 5C.7B.5 — Logs, métricas, alertas y presupuesto — 08/09/2026»). **5C.7B.6 = DEFINIDA / NO ABIERTA** (08/09/2026): resuelve formalmente la anomalía 3D.5/5C.7B.6 —3D.5 permanece cerrada/no abierta como antecedente histórico; sus obligaciones de backup/export/retención/identidad de backup se transfieren a 5C.7B.6, contenedor único, sin fases paralelas—; transfiere a la futura 6C la deuda de restore diferida de 3D.6; fija el principio «DR parcial demostrado ≠ DR integral de OXKIO» mientras existan stores solo-filesystem sin recuperación demostrada; y define la estructura 6A–6E (inventario, backup verificable con PRE-6B read-only de Neon obligatorio, restore aislado, DR+RPO/RTO, evidencia/cierre). No concede ninguna puerta humana ni abre 6A–6E; ver documento canónico, sección «Definición canónica 5C.7B.6 — Backup, export, restore y DR — 08/09/2026». **6A = CERRADA / PASS_WITH_LIMITATIONS READ-ONLY** (08/09/2026): inventario consolidado de autoridades reconciliado (Approval PostgreSQL/Neon crítico sin backup/restore, Security Inventory crítico sin cobertura, Secret Manager como credencial sin backup en claro, Memory/Project Registry/Executive Agenda/Strategic Memory locales sin cobertura, Operations/execution log con cobertura incidental vía Git); limitaciones registradas: `executionLog.json` y `executiveAgendaStore.json` trackeados en Git (cobertura incidental, no gobernada, no destrackeados en este cierre) y bloque histórico de Approval JSON V2 marcado SUPERSEDED por B6 sin borrar historia; UNKNOWN de Neon transferido íntegro al PRE-6B, sin bloquear 6A; no se declara DR completo de OXKIO; ver documento canónico, sección «Cierre documental 6A — Inventario, autoridad y criticidad — 08/09/2026». **PRE-6B = CERRADO / PASS READ-ONLY** (08/09/2026): verificación visual real del proyecto Neon `OXKIO` (organización `XANTALAL`) realizada directamente por José Antonio en la consola web ya autenticada, sin acceso automatizado — plan Free confirmado, región AWS Europe Central 1 (Fráncfort) confirmada, PostgreSQL 18 confirmado, ventana PITR real de 6 horas confirmada, sin snapshots actuales ni programados, creación manual disponible, programación avanzada requiere mejora de plan; uso puntual observado (0,66 CU-hrs, 32,21 MB storage, 178,67 kB history, 1,44 MB network) registrado sin extrapolar coste; decisión arquitectónica fijada para 6B: NEON NATIVO + PG_DUMP INDEPENDIENTE; PostgreSQL 18 en preview queda como riesgo tecnológico separado, no como parte del cierre DR; ver documento canónico, sección «Cierre documental PRE-6B — Verificación real Neon — 08/09/2026». **6B = DEFINIDA / NO ABIERTA** (08/09/2026): activo inicial Approval PostgreSQL/Neon; artefacto `pg_dump -Fc` unico por ejecución con nombre determinista; conexión unpooled con credencial efímera; dos identidades distintas y no confundibles: identidad ejecutora `oxkio-backup-prod@...` (candidata inventariada, sin permisos IAM asignados ni afirmados, no autentica directamente contra PostgreSQL) e identidad/rol PostgreSQL de backup (PG-BKP, no materializado, sin credencial ni privilegios demostrados, no se crea aquí); destino fuera de Neon y preferiblemente fuera del dominio administrativo de `oxkio-runtime-prod`, PENDIENTE DE DECISIÓN; cifrado en tránsito/reposo obligatorio; integridad vía SHA-256 y manifiesto no secreto; PASS futuro exige exit code 0, artefacto no vacío, checksum e inspección sin mutación, nunca solo por existir el archivo; retención mínima con más de una generación, PENDIENTE la cifra numérica; datos tratados como activo sensible; fail-closed ante cualquier fallo o exposición de secreto; puerta humana separada obligatoria para la ejecución real; 6B nunca restaura ni sustituye autoridad (eso es 6C); 8 pendientes registrados (incluyendo, como dos decisiones separadas, la identidad ejecutora y el rol/credencial PostgreSQL de backup) que no impiden definir 6B pero sí impiden ejecutarla; ver documento canónico, sección «Definición canónica 6B — Backup/export verificable — 08/09/2026». **5C.7B.6 sigue ABIERTA**, sin abrir 6B. **6C–6E = NO ABIERTAS. 5C.7B.7 = NO ABIERTA**; no se abre ninguna fase posterior. **Consolidación documental PRE-6B — Destino, retención e identidades (08/09/2026):** subfases de diseño pre-ejecución `PRE-6B.1B` (destino: candidato OVHcloud Object Storage Standard 3-AZ París, alternativa válida Scaleway Object Storage París, secundario Google Drive/One manual, Object Lock Governance no Compliance — **PASS READ-ONLY**), `PRE-6B.2` (retención: propuesta GFS ligero 7 diarios/4 semanales/3 mensuales como generaciones lógicas aproximadas, frecuencia diaria propuesta inicial pendiente de validar contra el RPO futuro de 6D, sin apoyarse en el PITR de Neon para justificarla, regla `OBJECT_LOCK_EXPIRY <= LIFECYCLE_EXPIRY` — **PASS READ-ONLY**) y `PRE-6B.3` (identidades: B.1 confirmado como identidad GCP de orquestación sin permisos IAM y distinta de cualquier rol PostgreSQL; B.2 diseñado como `PG-BKP-ROLE` NOLOGIN con `BYPASSRLS` aprobado como excepción de diseño justificada por `FORCE ROW LEVEL SECURITY` real en Approval, más `PG-BKP-LOGIN` LOGIN rotable, ninguno materializado; corregida la formulación previa sobre `pg_dump`+RLS: sin `BYPASSRLS` el comportamiento real es fallo cerrado con error, no exportación silenciosa de 0 filas; alcance ratificado como dominio Approval únicamente — **CERRADA / PASS READ-ONLY** tras ratificación humana explícita), todas distintas del `PRE-6B` singular ya cerrado (verificación Neon). Exclusivamente documental: sin SQL mutativo, sin roles PostgreSQL, sin `GRANT`/`REVOKE`, sin secretos, sin IAM/Neon/GCP mutativo, sin contratación OVH/Scaleway, sin bucket, sin `pg_dump`, sin backup. Ver documento canónico, sección «Consolidación documental PRE-6B — Destino, retención e identidades de backup — 08/09/2026». **Estado: 5C.7B.6 = ABIERTA; 6B = DEFINIDA / NO ABIERTA; 6C–6E = NO ABIERTAS; 5C.7B.7 = NO ABIERTA.** **Apertura 6B tramo OFFLINE — 6B.1 toolchain y 6B.2 wrapper `pg_dump` (09/09/2026):** bajo autorización humana explícita limitada a implementar y validar sin conexión real. **6B.1 = PASS**: PostgreSQL client tools locales verificados por ruta absoluta (`pg_dump`, `pg_restore`, `psql`, los tres 18.6), coincidiendo en versión mayor con el PostgreSQL 18 canónico de Neon; resuelve el pendiente (6) de la sección M solo en su vertiente local, no en la identidad ejecutora B.1. **6B.2 = PASS OFFLINE**: wrapper seguro de `pg_dump` implementado en `backend/services/backup/pg-dump-wrapper.js` reutilizando `secret-runtime` y el contrato anti-`connectionString` de `postgres-approval-factory`, sin subsistema paralelo — credencial efímera inyectada y confinada al entorno del proceso hijo construido por lista blanca, TLS `verify-full` y channel binding `require` impuestos por código y no debilitables, endpoint DIRECTO obligatorio con `-pooler` abortando sin autocorrección, formato custom con nombre determinista y salida fuera del repositorio/OneDrive/`.git` sin overwrite, `--no-owner`/`--no-privileges` no emitidos por decisión diferida a 6C, alcance dominio Approval únicamente con Mission Queue excluida y el modo `owner_resolved` reservado como contrato que falla como PENDIENTE, `ValidateOnly` sin lanzar `pg_dump` y devolviendo solo datos no secretos, fail-closed PRE y POST completo con SHA-256 y saneado de salidas, y `pg_restore --list` probado solo offline con fixture sintético; FOCAL 34/34 PASS y RELATED 211/211 PASS (2 SKIP preexistentes por entorno). Hallazgo nuevo: `verify-full` exige una CA raíz resoluble, mecanismo no verificado para la identidad ejecutora futura, pendiente pre-ejecución. Sin conexión a Neon, sin `pg_dump` real, sin `PG-BKP` creado, sin SQL mutativo, sin `GRANT`/`REVOKE`, sin `BYPASSRLS` real, sin secretos, sin IAM, sin OVHcloud, sin bucket y sin backup real. Ver documento canónico, sección «Apertura 6B tramo OFFLINE — 6B.1 toolchain y 6B.2 wrapper `pg_dump` — 09/09/2026». **Estado: 5C.7B.6 = ABIERTA; 6B = ABIERTA EN SU TRAMO OFFLINE; 6B.1 = PASS; 6B.2 = PASS OFFLINE; 6C–6E = NO ABIERTAS; 5C.7B.7 = NO ABIERTA.** **6B.3A — CA raíz + alcance dinámico Approval (09/09/2026):** estrictamente offline. CA: evidencia oficial (PostgreSQL 18, hilo pgsql-hackers abril 2025, docs Neon) confirma `sslrootcert=system` disponible desde libpq 16 pero no fiable en Windows sin almacén OpenSSL propio, corroborado por la propia Neon (documenta que Windows no ofrece un archivo de raíces CA utilizable); Neon usa ISRG Root X1 (Let's Encrypt), ampliamente presente en `ca-certificates` de Linux mantenido — decisión: system trust store condicionado por plataforma, recomendado para la identidad ejecutora real (Cloud Run/Linux), no forzado en Windows; wrapper añade `validateSslRootCert` fail-closed (ruta relativa o inexistente aborta) y publica solo una categoría, nunca la ruta real. Alcance dinámico: comparadas 5 estrategias, elegida manifiesto versionado (`approval-domain-manifest.js`, hoy solo `oxkio.approval_items`) + resolver inyectable con igualdad exacta de conjuntos — cualquier discrepancia, owner inesperado, fallo del resolver o resultado vacío es fail-closed, y ningún objeto de Mission Queue se acepta aunque venga del catálogo resuelto; sin manifiesto y resolver explícitos, `owner_resolved` sigue PENDIENTE por defecto. Privilegios de catálogo marcados como GAP documentado, no hecho canónico, por falta de cita oficial exacta. `validateOnly` pasó a async (cambio de contrato interno sin consumidores externos). FOCAL 47/47 PASS (antes 34); RELATED 224/224 PASS (2 SKIP preexistentes). **COUNCIL_REQUIRED = YES** sobre si `PGSSLROOTCERT=system` se adopta como CA por defecto de la ejecución real, dado que la imagen base concreta de Cloud Run no está verificada. Sin conexión a Neon, sin `pg_dump` real, sin SQL, sin `PG-BKP`, sin `BYPASSRLS` real, sin secretos, sin IAM, sin OVHcloud, sin bucket y sin backup real. Ver documento canónico, sección «6B.3A — CA raíz + alcance dinámico Approval — 09/09/2026». **Estado: 5C.7B.6 = ABIERTA; 6B = ABIERTA EN SU TRAMO OFFLINE; 6B.1 = PASS; 6B.2 = PASS OFFLINE; 6B.3A = PASS_WITH_LIMITATIONS OFFLINE; 6C–6E = NO ABIERTAS; 5C.7B.7 = NO ABIERTA.** **6B.3D — Variante offline de imagen de backup (09/09/2026):** consejo de 6B.3A/6B.3B/6B.3C resuelto en codigo — PGSSLROOTCERT=system adoptado como politica Linux sin root.crt propio, base pinneada por rama node:22-alpine3.24 (no por digest exacto todavia; digest de build registrado como evidencia: sha256:c610fcdf...3aa32). Dockerfile: dependencies reutilizada; nueva etapa compartida app; dos targets hermanos — runtime (HTTP, ultima etapa, sin cambio de contrato de build, sin postgresql18-client) y backup (nuevo, apk add --no-cache postgresql18-client sin fijar version, USER node final). Validado con Docker real (un unico pull, sin push): target HTTP sin pg_dump/pg_restore/psql; target backup con los tres en 18.6, Alpine real 3.24.1, ISRG Root X1 confirmado en el trust store real, sin certificado propio anadido, sin correr como root, sin secretos en env. Tests nuevos en deploy/: dockerfile.test.js (estatico, 10/10) y dockerfile-build.test.js (Docker real, auto-omitido si no disponible, 9/9). FOCAL 19/19 PASS; RELATED 71/71 PASS. Sin conexion a Neon, sin pg_dump real, sin backup, sin SQL, sin PG-BKP, sin BYPASSRLS, sin secretos, sin IAM, sin OVHcloud, sin bucket, sin push de imagen y sin deploy Cloud Run/GCP. Ver documento canonico, seccion «6B.3D — Variante offline de imagen de backup — 09/09/2026». **Estado: 5C.7B.6 = ABIERTA; 6B = ABIERTA EN SU TRAMO OFFLINE; 6B.1 = PASS; 6B.2 = PASS OFFLINE; 6B.3A = PASS_WITH_LIMITATIONS OFFLINE; 6B.3B = PASS_WITH_LIMITATIONS READ-ONLY; 6B.3C = PASS_WITH_LIMITATIONS READ-ONLY; 6B.3D = PASS OFFLINE; 6C–6E = NO ABIERTAS; 5C.7B.7 = NO ABIERTA.** **6B.3E — Runner offline del backup (09/09/2026):** nuevo backend/services/backup/backup-runner.js, sin reimplementar nada — delega en createPgDumpWrapper() y consume EXPECTED_APPROVAL_TABLES del manifiesto directamente. CLI reutiliza el patron ya existente de persistence-poc-runner.js. Contrato de proveedor de secreto {async getBackupCredential()} y de catalogo resolveApprovalCatalog (reutilizado del wrapper); sin ninguno inyectado, falla cerrado antes de tocar nada. Modos validate-only (default, nunca abre red) y execute (contrato interno, exige mode==execute Y allowExecute===true simultaneos). TLS no configurable desde fuera, PGSSLROOTCERT=system por defecto. Integracion Docker sin ningun cambio de Dockerfile (el runner ya vive en el target backup via la etapa compartida app); invocado explicitamente dentro del contenedor real sin providers reales inyectados -> exit code 1, runner_secret_provider_missing, cero literales postgres://PASSWORD en la salida, confirmado empiricamente. Confirmado que ningun CMD/ENTRYPOINT invoca el runner automaticamente. FOCAL 98/98 PASS (30 nuevos de runner + 47 wrapper + 10 dockerfile estatico + 11 dockerfile build con 2 nuevos de integracion real). RELATED 177/177 PASS (2 SKIP preexistentes). Sin conexion a Neon, sin pg_dump real, sin backup, sin SQL, sin PG-BKP, sin BYPASSRLS, sin GRANT/REVOKE, sin secretos reales, sin IAM, sin OVHcloud, sin bucket, sin subida de artefactos, sin Cloud Run Job y sin deploy GCP. Ver documento canonico, seccion «6B.3E — Runner offline del backup — 09/09/2026». **Estado: 5C.7B.6 = ABIERTA; 6B = ABIERTA EN SU TRAMO OFFLINE; 6B.1 = PASS; 6B.2 = PASS OFFLINE; 6B.3A = PASS_WITH_LIMITATIONS OFFLINE; 6B.3B = PASS_WITH_LIMITATIONS READ-ONLY; 6B.3C = PASS_WITH_LIMITATIONS READ-ONLY; 6B.3D = PASS OFFLINE; 6B.3E = PASS OFFLINE; 6C–6E = NO ABIERTAS; 5C.7B.7 = NO ABIERTA.**
